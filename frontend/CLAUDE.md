...
> **note (이 프로젝트 = 관리시스템)**: 세션 방식이라 **JWT 첨부 대신 `withCredentials`로 세션 쿠키를 전송**한다.
> authStore는 토큰이 아니라 로그인 사용자 정보를 담는다. 근거: `.claude/context/security-policy.md`.

| | 처리 |
|---|---|
| **한다** | 세션 쿠키 전송(withCredentials) · 응답 해체 · 401 로그아웃·이동 · 403·5xx·네트워크 공통 토스트 · `BusinessError` 변환 |
